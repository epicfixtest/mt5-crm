...
$pageTitle = 'จัดการช่องทางการติดต่อ';
ob_start();

// บันทึกข้อมูล
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['content'])) {
    $stmt = $pdo->prepare("UPDATE contact_info SET content = ? WHERE id = 1");
...
$contactContent = $stmt->fetchColumn();
?>

<div class="max-w-3xl mx-auto bg-white dark:bg-gray-800 p-6 rounded shadow">
    <h1 class="text-2xl font-bold mb-4">📬 แก้ไขช่องทางการติดต่อ</h1>
    ...
    <form method="post">
        <textarea name="content" rows="10" class="w-full p-4 rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-800 dark:text-white"><?= htmlspecialchars($contactContent) ?></textarea>
